...
class MULTIHEAD_POLY(nn.Module):
    def __init__(self, nc=1, anchors=(), ch=(), out_t=[1, 17, 9]):  # detection layer
        super(MULTIHEAD_POLY, self).__init__()
        ...
        for i, in_channels in enumerate(ch):
            self.out_convs += [
                nn.Conv2d(in_channels=in_channels, out_channels=(self.out_t[i]) * self.naxs, kernel_size=1)
            ]
    ...
    def forward(self, x):
        for i in range(self.nl):
            ...
            x[i] = x[i].view(bs, self.naxs, (self.out_t[i]), grid_y, grid_x).permute(0, 1, 3, 4, 2).contiguous()
...
class SINGLE_HEAD(nn.Module):
    ...
    def forward(self, x):
        # Apply convolution and reshape for the first layer only
        x[0] = self.out_convs[0](x[0])
        bs, _, grid_y, grid_x = x[0].shape
        x[0] = x[0].view(bs, self.naxs, (5 + self.nc), grid_y, grid_x).permute(0, 1, 3, 4, 2).contiguous()

        # Return only the output from the first layer
        return x[0]
    
class SINGLE_HEAD_POLYGON(nn.Module):
    def __init__(self, nc=80, anchors=(), ch=(), out_t=9):
        super(SINGLE_HEAD_POLYGON, self).__init__()
        # self.nc = nc  # number of classes
        # self.nl = len(anchors)  # number of detection layers
        # self.naxs = len(anchors[0])
        ...
        for in_channels in ch:
            self.out_convs += [
                nn.Conv2d(in_channels=in_channels, out_channels=(self.out_t) * self.naxs, kernel_size=1) # 4points (each 2 points) + 1 objectness loss
            ]
    ...
    def forward(self, x):
        ...
        # Reshape to output 8 values for polygon vertices
        x[0] = x[0].view(bs, self.naxs, self.out_t, grid_y, grid_x)  # (b, B, 8, S, S)

        # Rearrange dimensions for consistency with model requirements
        x[0] = x[0].permute(0, 1, 3, 4, 2).contiguous()  # (b, B, S, S, 8)
...
class YOLOV5m(nn.Module):
    def __init__(self, first_out, nc=80, anchors=(),
        ...
        self.head = MULTIHEAD_POLY(nc=nc, anchors=anchors, ch=ch, out_t=[1, 9, 17]) # for house polygons
